...
export default function Product3DViewer({ name, image, frames = [], compact = false }: Product3DViewerProps) {
  ...
  const handleKeyDown = (event: React.KeyboardEvent<HTMLDivElement>) => {
    ...
    if (event.key === 'Home') return resetRotation(event);
  ...
  return (
    <div
      ...
      role="group"
      tabIndex={0}
      aria-label={`${name} 360 degree product viewer. Drag left or right to rotate through the supplied frame sequence.`}
    ...
    >
      ...
      {visible && frames.length > 1 && preloadState !== 'ready' && (
        <div className="pointer-events-none absolute left-3 top-3 rounded-full border border-white/15 bg-black/55 px-3 py-1 text-[8px] font-bold uppercase tracking-[.16em] text-white/75 backdrop-blur">Loading 360°</div>
      )}
      <button
        type="button"
